...
input.addEventListener('keyup', (e) => {
  ...
  let inputValue = e.target.value.toUpperCase(); // строка из инпута
  inputValue = inputValue.trim(); // строка из инпута без пробелов
  let span = null;
  let newDiv = document.createElement('div');
  for (let i = 0; i < countries.length; i++) {
    
    let country = countries[i].toUpperCase();
    country = country.slice(0, inputValue.length);

    if (inputValue === country && inputValue !== ""){
      
      div.style.display = 'block';
      span = document.createElement('span');
      span.classList.add('country');
      span.innerHTML += countries[i] + " <br>";
    ...
    else if (inputValue === country || inputValue === "" && div.innerHTML !== '') {
        div.innerHTML = '';
    }         
  }   
}); 
